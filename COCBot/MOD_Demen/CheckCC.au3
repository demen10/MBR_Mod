	...
	For $i = 0 To 7
		...
		If _ColorCheck(_GetPixelColor(Round(30 + 72.8 * $i + $Spell_Offset, 0), 508, True), Hex(0xCFCFC8, 6), 15) = True Then
			If $i < 6 Then
				...
				If $g_iDebugSetlog = 1 Then SetLog("No more CC troop, jump to slot: " & $i + 1, $COLOR_DEBUG) ;Debug
				ContinueLoop
			Else
				ExitLoop ; Slot 6 has no spell
				If $g_iDebugSetlog = 1 Then SetLog("No CC spell, quit checking", $COLOR_DEBUG) ;Debug
			EndIf
		EndIf
		...
		Else ; name of first file found
			...
			If $iFound - $iExpect > 0 Then
				...
				If $g_iDebugSetlog = 1 Then SetLog("Expected: " & $aResult[1] & " x" & $iExpect & ". Should remove: x" & $aToRemove[$i], $COLOR_DEBUG) ;Debug
				$bNeedRemoveCC = True
			EndIf
		EndIf
	Next
