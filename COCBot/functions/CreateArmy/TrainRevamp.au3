...
Func CompareCCSpellWithGUI($CCSpell1, $CCSpell2, $CastleCapacity)
	...
	For $Mode = $DB To $LB
		...
		If BitOR($g_aiSearchCastleSpellsWaitRegular[$Mode], $g_aiSearchCastleSpellsWaitDark[$Mode]) > 0 Then
			...
			If ($sCCSpell = $CCSpell1[0][0] Or $sCCSpell = "Any") Then ; fix bug on checking Slot2 - Demen
			...
			If $bCheckCCSpell2 Then ; Castle Spells capacity is 2
                ...
                If $CCSpell2 = "" Then ExitLoop	; fix bug on checking Slot2 - Demen
...
Func OpenTrainTabNumber($iTabNumber, $sWhereFrom)
	...
	If IsTrainPage() Then
		...
		If _Sleep(700) Then Return			; Too slow with wait time 1.5s. Reduce to 0.7s. - SmartTrain - Demen
...
Func TrainArmyNumber($Army, $iMultiClick = 1) ; QuickTrainCombo (Checkbox) - Demen
	...
	If $areThereDonTroop > 0 Then
		...
		For $i = 0 To UBound($avDefaultTroopGroup, 1) - 1
			...
			If $avDefaultTroopGroup[$i][4] > 0 Then
				...
				If $RemainTrainSpace[0] = $RemainTrainSpace[1] And $ichkSmartTrain <> 1 Then ; army camps full	;; SmartTrain - Demen
				...
				If $avDefaultTroopGroup[$i][2] * $avDefaultTroopGroup[$i][4] <= $RemainTrainSpace[2] Or $ichkSmartTrain = 1 Then ; Troopheight x donate troop qty <= avaible train space ;; ;; SmartTrain - Demen
